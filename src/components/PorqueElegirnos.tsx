        ...
        {cards.map((c, idx) => (
          ...
          >
            <img
              src={c.image}
              alt={c.title}
              className="w-full lg:w-[520px] h-auto lg:h-[220px] object-cover rounded-xl"
            />
            <div
              className={`bg-g-10 p-6 lg:p-8 w-full lg:max-w-[1040px] min-h-[160px] space-y-2 text-justify flex flex-col justify-center ${
                idx % 2 === 1 ? 'rounded-l-lg' : 'rounded-r-lg'
              }`}
            >
              <p className="text-primary font-bold text-[20px] lg:text-[22px]">{c.title}</p>
              <p className="text-g-50 text-[17px] lg:text-[17px]">{c.description}</p>
            </div>
          </div>
        ))}
      </div>
