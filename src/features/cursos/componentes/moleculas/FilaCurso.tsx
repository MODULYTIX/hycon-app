  ...
  return (
    <li className={`grid gap-3 px-4 py-4 transition-colors hover:bg-hy-5/60 sm:px-5 lg:items-center lg:gap-4 ${COLUMNAS_CURSO}`}>
      <div className="flex min-w-0 gap-3.5">
        <Miniatura url={portada} iconoReserva="solar:diploma-linear" clase="h-16 w-24" />
        ...
        <div className="min-w-0 flex-1 self-center">
          ...
          {curso.youtubeId ? (
            ...
            curso.videoUrl && (
              // Links antiguos que no son de YouTube: se avisa en lugar de reproducir
              <p className="mt-1 inline-flex items-center gap-1 text-[12.5px] font-medium text-y-70">
                <Icon icon="solar:danger-triangle-linear" width="14" height="14" aria-hidden />
                Link de video no valido
              </p>
            )
          )}
        </div>
